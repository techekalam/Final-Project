...
SELECT u.id, 'CU-2026-001', 'Jane Doe', 'student@cavendish.ac.ug', '+256 700 000000', 'Computer Science', 'BSc Computer Science', 2
FROM users u WHERE u.email = 'student@cavendish.ac.ug'
ON CONFLICT (student_id) DO NOTHING;

INSERT INTO students (user_id, student_id, name, email, phone, faculty, program, year_of_study)
SELECT u.id, 'CU-2026-002', 'John Smith', 'john@cavendish.ac.ug', '+256 700 111111', 'Business Administration', 'BBA', 1
...
WHERE s.student_id = 'CU-2026-001' AND c.code IN ('CS101', 'CS102', 'MTH101')
...
FROM students s WHERE s.student_id = 'CU-2026-001';

-- Sample Results
INSERT INTO results (student_id, course_id, grade, score, semester, academic_year)
SELECT s.id, c.id, 'A', 85, 'Semester 1', '2025/2026'
FROM students s, courses c WHERE s.student_id = 'CU-2026-001' AND c.code = 'CS101';

INSERT INTO results (student_id, course_id, grade, score, semester, academic_year)
SELECT s.id, c.id, 'A+', 92, 'Semester 1', '2025/2026'
FROM students s, courses c WHERE s.student_id = 'CU-2026-001' AND c.code = 'CS102';

INSERT INTO results (student_id, course_id, grade, score, semester, academic_year)
SELECT s.id, c.id, 'B+', 78, 'Semester 1', '2025/2026'
FROM students s, courses c WHERE s.student_id = 'CU-2026-001' AND c.code = 'MTH101';
